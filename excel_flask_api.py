...
def excel_to_pdf(excel_file_path, pdf_file_path):
    ...
    for i in range(doc.Pages.Count):
        ...
        replacer.ReplaceAllText("Evaluation Warning : The document was created with Spire.XLS for Python", "")
...
def generate_timesheet_report():
    ...
    thick_side = Side(border_style="thick", color="000000")  # Thick border style
    thin_side = Side(border_style="thin", color="000000")  # Thick border style   
    ...
    ws['B4'] = 'Project Report ' +  data['project_name'] if data['project_name'] else None
    ...
    thin_red_bottom_border = Border(bottom=Side(style='thin', color='000000'))
...
# Apply border to specified cells
    ...
    for day in range(1, num_days_in_month + 1):
        ...
        for task in data['date_wise_tasks']:
            task_day, task_month, task_year = map(int, task['date'].split('/'))  # Parsing task date
        ...
        is_weekend = weekday >= 5  # Saturday and Sunday are represented by 5 and 6 respectively in calendar module
        ...
        if tasks_for_day:
            ...
            for sub_task in tasks_for_day['tasks']:
                cell = ws[f'C{row}']
                cell_value = cell.value + ', ' + sub_task['name'] + ' ' + sub_task['description'] if cell.value else sub_task['name'] + ' ' + sub_task['description']
                # Enable word wrapping for the cell
                cell.alignment = Alignment(wrapText=True)
                cell.value = cell_value
                    
                # Calculate the required row height based on wrapped text and column width
                cell_wrap_height = (len(cell_value) // (ws.column_dimensions['C'].width // 7)) * 2  # Approximate height per line
        ...
        if not tasks_for_day and is_weekend:
            for col in ['B', 'C', 'D']:
                ws[f'{col}{row}'].fill = PatternFill(start_color=weekend_fill_color, end_color=weekend_fill_color, fill_type="solid")
            if 'total_days_worked' in data and data['total_days_worked'] != 0:
                ws[f'E{row}'].fill = PatternFill(start_color=weekend_fill_color, end_color=weekend_fill_color, fill_type="solid")


        row += 1

    ws[f'C{row+2}'] = 'Summe (in h)'
    ws[f'C{row+2}'].alignment = Alignment(horizontal='right', vertical='center')
    ...
    ws[f'D{row+2}'].alignment = Alignment(horizontal='center', vertical='center')
    ...
    for row_item in range(19, row + 1):
        ...
        ws[f'D{row_item}'].border = Border(left=thin_side,bottom=thin_side,right=thick_side,top=thin_side)
        if 'total_days_worked' in data and data['total_days_worked'] != 0:
            ws[f'D{row_item}'].border = Border(left=thin_side,bottom=thin_side,right=thin_side,top=thin_side)
            ws[f'E{row_item}'].border = Border(left=thin_side,bottom=thin_side,right=thick_side,top=thin_side)
            
        if row_item == row:
            ws[f'B{row_item}'].border = Border(left=thick_side, bottom=thick_side, right=thin_side, top=thin_side)
            ws[f'C{row_item}'].border = Border(left=thin_side, bottom=thick_side, right=thin_side, top=thin_side)
            ws[f'D{row_item}'].border = Border(left=thin_side, bottom=thick_side, right=thick_side, top=thin_side)
            if 'total_days_worked' in data and data['total_days_worked'] != 0:
    ...
    excel_file_path = os.path.join(excel_folder_path, f"{unique_filename}.xlsx")
